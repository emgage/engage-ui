    ...
    return(
      <React.Fragment key={index}>
        <TableRow theme={theme}>
          ...
          { rowAction && !rowActionLeft ? <TableData componentClass={theme.lastData} componentStyle={{ float: 'right', width: '72px', textAlign: 'center' }}>{item.isRowLoading && <Spinner componentSize="small" componentColor="disabled" />} <RowAction actionInProgress={actionInProgress && !!item.processing} isRowLoading={item.isRowLoading} actionConfig={rowAction} data={item} theme={theme} /> </TableData> : '' }
        </TableRow>
        { renderBanner &&
        <TableRow>
