...
describe('<Choice />', () => {
  it('uses the id as the for attribute of a label', () => {
    const element = mount(<Choice componentId="MyChoice" label="Label" disabled={false} labelHidden={false} />);
  ...
  it('renders the content as a child of the label', () => {
    ...
    const element = mount(<Choice componentId="MyChoice" label="Label" disabled={false} labelHidden={false} ><MYCOMPONENT /></Choice>);
    const label = element.find('label');
