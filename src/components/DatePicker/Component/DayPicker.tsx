...
export function calculateDimension(el: any, axis: any, borderBox: boolean = false, withMargin: boolean = false) {
  if (!el) {
    return 0;
  }
  ...
  const style: any = (!borderBox || withMargin) ? window.getComputedStyle(el) : null;
...
function getMonthHeight(el: any) {
...
export interface State {
  onKeyboardShortcutsPanelClose: any,
  isTouchDevice: any,
...
export interface Props {
  ...
  onBlur?: any,
  isRTL?: any,
  getFirstFocusableDay?: any,
  numberOfMonths?: any,
  onMultiplyScrollableMonths?: any,
  orientation?: any,
...
class DayPicker extends React.Component<Props, State> {
  ...
  container: any;
  dayPickerWidth: any;
  hasSetInitialVisibleMonth: boolean;
  constructor(props: any) {
    super(props);

    const currentMonth: any = props.hidden ? moment() : moment();   //props.initialVisibleMonth()
    ...
    this.state = {
      ...
      isTouchDevice: false,  //isTouchDevice()
  ...
  onKeyDown(e: any) {
    ...
    const activeElement: any = getActiveElement();
  ...
  onPrevMonthClick(nextFocusedDate: any, e: any) {
  ...
  onNextMonthClick(nextFocusedDate: any, e: any) {
    const { isRTL } = this.props;
  ...
  getFocusedDay(newMonth: any) {
    ...
    if (newMonth && (!focusedDate || !isDayVisible(focusedDate, newMonth, numberOfMonths,null))) {
  ...
  maybeTransitionNextMonth(newFocusedDate: any) {
    ...
    const isNewFocusedDateVisible = isDayVisible(newFocusedDate, currentMonth, numberOfMonths,null);
    if (newFocusedDateMonth !== focusedDateMonth && !isNewFocusedDateVisible) {
      this.onNextMonthClick(newFocusedDate, null);
      return true;
    }
  ...
  maybeTransitionPrevMonth(newFocusedDate: any) {
    const { numberOfMonths } = this.props;
    const { currentMonth, focusedDate } = this.state;
    ...
    const isNewFocusedDateVisible = isDayVisible(newFocusedDate, currentMonth, numberOfMonths,null);
    if (newFocusedDateMonth !== focusedDateMonth && !isNewFocusedDateVisible) {
      this.onPrevMonthClick(newFocusedDate,null);
      return true;
    }
  ...
  multiplyScrollableMonths(e: any) {
  ...
  isHorizontal() {
    return this.props.orientation === Constants.HORIZONTAL_ORIENTATION;
  }

  isVertical() {
    return this.props.orientation === Constants.VERTICAL_ORIENTATION ||
      this.props.orientation === Constants.VERTICAL_SCROLLABLE;
  }
  ...
  translateFirstDayPickerForAnimation(translationValue: any) {
    const { isRTL } = this.props;
  ...
  openKeyboardShortcutsPanel(onCloseCallBack: any) {
    this.setState({
  ...
  renderNavigation() {
    ...
    if (orientation === Constants.VERTICAL_SCROLLABLE) {
      ...
      onNextMonthClick = (e: any) => { this.onNextMonthClick(null, e); };
    }

    return (
      <DayPickerNavigation
        onPrevMonthClick={(e: any) => { this.onPrevMonthClick(null, e); }}
  ...
  renderWeekHeader(index: any) {
    const { daySize, orientation } = this.props;
    const { calendarMonthWidth } = this.state;
    const verticalScrollable = orientation === Constants.VERTICAL_SCROLLABLE;
