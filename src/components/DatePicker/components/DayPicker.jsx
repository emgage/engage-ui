...
export function calculateDimension(el, axis, borderBox = false, withMargin = false, theme) {
  if (!el) {
    return 0;
  }
...
function getMonthHeight(el, theme) {
  ...
  const grid = el.querySelector(`.${themedGrid}`);
...
class DayPicker extends React.Component {
  constructor(props) {
    ...
    // this.getMonthHeightByIndex = this.getMonthHeightByIndex.bind(this);
  ...
  initializeDayPickerWidth() {
    const themedStyle = this.props.theme['CalendarMonth'];
    if (this.calendarMonthGrid) {
      ...
      if (calendarMonthGridDOMNode) {
        this.dayPickerWidth = calculateDimension(
          calendarMonthGridDOMNode.querySelector(`.${themedStyle}`),
          'width',
          true,
        );
      }
  ...
  updateStateAfterMonthTransition() {
    ...
    if (this.calendarMonthGrid) {
      ...
      if (calendarMonthGridDOMNode) {
        ...
        const styledTheme = this.props.theme['CalendarMonth']
        applyTransformStyles(
          calendarMonthGridDOMNode.querySelector(`.${styledTheme}`),
          'none',
        );
      }
    }
  ...
  adjustDayPickerHeight() {
    ...
    console.log('calculateDimension:', calculateDimension(this.transitionContainer, 'height', false, false))
    console.log('transitioncontainer:', this.transitionContainer);
    if (newMonthHeight !== calculateDimension(this.transitionContainer, 'height', false, false)) {
  ...
  renderWeekHeader(index) {
    const { daySize, orientation, theme } = this.props;
    ...
    return (
      <div
        className={theme["DayPicker__week-header"]}
        key={`week-${index}`}
        style={style}
      >
        <ul>
  ...
  render() {
    ...
    const dayPickerClassNames = cx(theme['DayPicker'], {
      ...
      [theme['DayPicker--portal']]: withPortal,
    });

    const transitionContainerClasses = cx(theme['transition-container'], {
...
export default themr(DATEPICKER, baseTheme)(DayPicker);
