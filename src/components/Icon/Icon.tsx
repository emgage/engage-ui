...
const icon = ({
  ...
  return (
    <span className={className} style={componentStyle} aria-label={accessibilityLabel} onClick={(event) => { onClick ? onClick((callbackValue ? callbackValue : event)) : ''; }} tabIndex={tabIndex}>
      {contentMarkup}
    </span>
  );
};
